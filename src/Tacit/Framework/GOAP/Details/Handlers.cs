...
[Serializable] public class Handlers {
    ...
    public void OnResult(S s, string c = null) {
        switch (s) {
            case S.Failed:
                Do(s, c, OnFail, warnOnFail);
                break;
            case S.MaxIterExceeded:
                Do(s, c, OnMaxIterOverflow, warnOnOverflow);
                break;
            case S.CapacityExceeded:
                Do(s, c, OnCapacityOverflow, warnOnOverflow);
                break;
        }
    }

    private void Do(S s, string c, Policy policy, bool warn) {
        switch (policy) {
            case Policy.Stop:
                if (warn) Warn(s, c);
                break;
...
#else
    private void Warn(S s, string c) {
        Console.WriteLine($"{c}: {s}");
    }
    private void Err(S s, string c) {
        Console.WriteLine($"{c}: {s}");
    }
#endif
